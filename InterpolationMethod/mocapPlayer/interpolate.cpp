...
int main(int argc, char **argv) 
{
  ...
  //////// Used for output certain rotation angles of certain frames///////////
  /*
  // For lfmur X axis---frame 600 to 800
  printf("Rotation angle output:\n");
  Skeleton* temptSkeleton = pOutputMotion->GetSkeleton(); 
  int tempt = 0;
  for(int f=0; f < (pOutputMotion->GetNumFrames()); f++)
  {
    tempt++;
    Bone * bone = temptSkeleton->getRoot();
    int numbones = temptSkeleton->numBonesInSkel(bone[0]);

    for(int j = 2; j < numbones; j++)
    {
      if(tempt>=599 && tempt<=799 )
      {
        std ::string target = bone[j].name;
        if(target== "lfemur")
        {
          Posture* interpolatedPosture = pOutputMotion->GetPosture(tempt);
          double output = interpolatedPosture->bone_rotation[j].p[0];
          printf("%.4f\n", output);
        }
      }
    } 
  ...
  /*
  // For Root z axis---frame 200 to 500
  printf("Rotation angle output:\n");
  Skeleton* temptSkeleton = pOutputMotion->GetSkeleton(); 
  int tempt = 0;
  for(int f=0; f < (pOutputMotion->GetNumFrames()); f++)
  {
    ...
    int numbones = temptSkeleton->numBonesInSkel(bone[0]);
      ...
      if(tempt>=199 && tempt<=499 )
      {
        int root = Skeleton::getRootIndex();
        //printf("%d",root);
        Posture* interpolatedPosture = pOutputMotion->GetPosture(tempt);
        double output = interpolatedPosture->bone_rotation[root].p[2];
        printf("%.5f\n", output);
      }
  }*/
  return 0;
